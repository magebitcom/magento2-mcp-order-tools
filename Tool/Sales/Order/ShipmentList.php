...
/**
 * MCP tool `sales.order.shipments` — list shipments for an order.
 */
class ShipmentList implements ToolInterface
{
    public const TOOL_NAME = 'sales.order.shipments';
    public const ACL_RESOURCE = 'Magebit_McpOrderTools::tool_sales_order_shipments';
    ...
    {
        return 'List Shipments for Order';
    }

    /** @inheritDoc */
    public function getDescription(): string
    {
        return 'List every shipment attached to the given order (by entity or '
    ...
    public function execute(array $arguments): ToolResultInterface
    {
        ...
        $criteria = $this->searchCriteriaBuilder
            ->addFilter(ShipmentInterface::ORDER_ID, (int) $order->getEntityId())
        ...
        foreach ($result->getItems() as $shipment) {
            ...
            foreach ($plan as $resolver) {
                $row[$resolver->getKey()] = $resolver->resolve($shipment, $arguments);
            }
            $rows[] = $row;
        }
        ...
        if ($json === false) {
            throw new LocalizedException(__('Failed to encode shipment list as JSON.'));
        }
